...
class DutyBot:
    ...
    async def trigger_oncall_duty(self) -> bool:
        if self._session_task and not self._session_task.done():
            LOGGER.warning("Cannot trigger on-call duty because a reminder session is already in progress")
            return False
        task = asyncio.create_task(self._notify_today())
        self._set_session_task(task)
        return True

    def _set_session_task(self, task: asyncio.Task[None]) -> None:
        self._session_task = task

        def _clear_session_task(done_task: asyncio.Task[None]) -> None:
            if self._session_task is done_task and done_task.done():
